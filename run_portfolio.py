...
from data.db import daily_prices, get_engine, initialise_schema  # noqa: E402
...
from portfolio.state import get_nav, load_positions  # noqa: E402
...
def _load_prices(
    conn, tickers: list[str], score_date: str, lookback: int = 130
    ...
    rows = conn.execute(
        ...
        .where(daily_prices.c.ticker.in_(tickers + ["SPY"]) & (daily_prices.c.date <= score_date))
...
def _print_portfolio_summary(portfolio_df: pd.DataFrame, betas: pd.Series, nav: float) -> None:
    ...
    if "sector" in portfolio_df.columns:
        sect = portfolio_df.groupby("sector")["weight"].sum().sort_values(key=abs, ascending=False)
...
def _print_trade_list(trades: pd.DataFrame) -> None:
    ...
    if active.empty:
        ...
        return
    print(
        f"  {'#':<4} {'Ticker':<8} {'Action':<8} {'Price':>9} {'Cur Shrs':>10}"
    ...
    for _, row in active.iterrows():
        ...
        print(
            f"  {row['priority']:<4} {row['ticker']:<8} {row['action']:<8}"
            f" {price:>9.2f}"
...
def _print_current(conn) -> None:
    ...
    for _, row in positions.sort_values("direction").iterrows():
        print(
            f"{row['ticker']:<8} {row.get('direction', ''):<6}"
...
def run(args) -> None:
    ...
    with engine.connect() as conn:
        ...
        prices = _load_prices(
            ...
            all_tickers,
            score_date,
            lookback=config.get("portfolio", {}).get("mvo", {}).get("cov_lookback_days", 130) + 10,
        )
