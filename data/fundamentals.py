...
def _extract_statement_data(
    ...
    total_equity = g(balance, ["Total Stockholders Equity", "Stockholders Equity",
                               "Common Stock Equity"])
    ...
    buybacks = g(cashflow, ["Repurchase Of Capital Stock", "Repurchase Common Stock",
    ...
    return {
        "revenue": revenue, "gross_profit": gross, "operating_income": op_income, "ebit": ebit,
        "net_income": net_income, "rd_expense": rd, "total_assets": total_assets,
        "total_liabilities": total_liab, "total_equity": total_equity, "cash": cash,
        "total_debt": total_debt, "current_assets": cur_assets, "current_liabilities": cur_liab,
        "accounts_receivable": ar, "retained_earnings": ret_earn, "shares_outstanding": shares,
        "dividends_paid": div_paid, "cfo": cfo, "capex": capex, "fcf": fcf, "buybacks": buybacks,
    }
...
def _compute_ratios(d: dict, prev: dict | None, prev_yoy: dict | None) -> dict:
    ...
    return {
        "roe": roe, "roa": roa, "gross_margin": gross_margin, "operating_margin": op_margin,
        "net_margin": net_margin, "revenue_growth_yoy": rev_yoy, "revenue_growth_qoq": rev_qoq,
        "earnings_growth_yoy": ni_yoy, "earnings_growth_qoq": ni_qoq,
        "debt_to_equity": debt_equity, "current_ratio": current_ratio,
        "ar_to_revenue": ar_to_rev, "cfo_to_ni": cfo_to_ni, "accruals_ratio": accruals,
        "working_capital": working_cap, "asset_turnover": asset_turnover,
    }


def _upsert_period(conn: sa.engine.Connection, ticker: str, period_type: str,
                   period_end: str, raw: dict, ratios: dict) -> None:
    conn.execute(
...
def _process_ticker_yfinance(conn: sa.engine.Connection, ticker: str) -> int:
    ...
    for period_type, inc_df, bal_df, cf_df in [
        ...
        for i, raw in enumerate(period_data):
            ...
            period_end = pd.Timestamp(col).strftime("%Y-%m-%d") if hasattr(col, "strftime") else str(col)[:10]
...
def _process_ticker_fmp(conn: sa.engine.Connection, ticker: str, api_key: str) -> int:
    ...
    for period_type, fmp_period, limit in [
        ...
        inc_rows = _fmp_get("income-statement",        api_key, {"symbol": ticker, "period": fmp_period, "limit": limit})
        bal_rows = _fmp_get("balance-sheet-statement", api_key, {"symbol": ticker, "period": fmp_period, "limit": limit})
        cf_rows  = _fmp_get("cash-flow-statement",     api_key, {"symbol": ticker, "period": fmp_period, "limit": limit})
...
def _is_fresh(conn: sa.engine.Connection, ticker: str, max_age_days: int) -> bool:
    ...
    result = conn.execute(
        sa.select(sa.func.max(fundamentals.c.updated_at))
        .where(fundamentals.c.ticker == ticker)
    ).scalar()
    if result is None:
        return False
    try:
...
def update_fundamentals(
    ...
    logger.info("Fundamentals complete — %d periods stored across %d tickers",
                stored_total, updated)
    return summary
