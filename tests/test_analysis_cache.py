...
from analysis.cache import AnalysisCache, _utcnow_iso
...
class TestGetMiss:
    ...
    def test_expired_entry_returns_none(self, tmp_db):
        ...
        tmp_db.execute(analysis_results.insert().values(
            analyzer="earnings", ticker="AAPL", artifact_id="AAPL_2024-03-31",
            model="gpt-4o", result_json='{"score": 7}',
    ...
    def test_different_artifact_id_returns_none(self, tmp_db):
        cache = AnalysisCache(tmp_db)
        cache.set("earnings", "AAPL", "AAPL_2024-03-31", "gpt-4o",
                  {"score": 7}, _fake_usage(), 0.001)
...
class TestSet:
    def test_upsert_overwrites_existing(self, tmp_db):
        cache = AnalysisCache(tmp_db)
        cache.set("earnings", "AAPL", "AAPL_2024-03-31", "gpt-4o",
                  {"score": 5}, _fake_usage(), 0.001)
        cache.set("earnings", "AAPL", "AAPL_2024-03-31", "gpt-4o",
                  {"score": 9}, _fake_usage(), 0.001)
        hit = cache.get("earnings", "AAPL", "AAPL_2024-03-31")
        assert hit["score"] == 9
...
class TestEviction:
    def test_evicts_expired_rows(self, tmp_db):
        ...
        for i in range(3):
            tmp_db.execute(analysis_results.insert().values(
                analyzer="risk", ticker=f"T{i}", artifact_id=f"T{i}_x",
                model="gpt-4o", result_json='{}',
...
class TestArtifactIds:
    ...
    def test_risk_artifact_id(self):
        assert AnalysisCache.artifact_id_risk("GOOG", "0001234567-24-000001") == "GOOG_0001234567-24-000001"
