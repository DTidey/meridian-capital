...
def test_compute_net_beta_long():
    ...
    import pandas as pd
    hist = pd.DataFrame([
        {"ticker": "A", "direction": "LONG", "market_value": 500_000, "sector": "Tech", "weight": 0.05},
        {"ticker": "B", "direction": "LONG", "market_value": 500_000, "sector": "Tech", "weight": 0.05},
    ])
...
def test_compute_net_beta_long_short():
    ...
    hist = pd.DataFrame([
        {"ticker": "A", "direction": "LONG",  "market_value": 500_000, "sector": "Tech", "weight": 0.05},
        {"ticker": "B", "direction": "SHORT", "market_value": 300_000, "sector": "Tech", "weight": 0.03},
    ])
    result = _compute_net_beta(hist)
...
def test_brinson_zero_when_weights_match_benchmark():
    ...
    sectors = ["Tech", "Finance", "Health", "Energy", "Industrials",
               "Communication", "Consumer Disc", "Consumer Stap", "Materials", "Real Estate", "Utilities"]
    n = len(sectors)
    equal_w = 1.0 / n

    hist = pd.DataFrame([
        {"ticker": f"T{i}", "direction": "LONG", "market_value": equal_w * 1e6,
         "sector": sectors[i], "weight": equal_w}
        for i in range(n)
    ])
    ...
    etf_prices = {f"ETF{i}": 100.0 for i in range(n)}
