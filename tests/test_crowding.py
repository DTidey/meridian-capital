...
from factors.crowding import detect, _compute_factor_returns, _load_history
...
def _insert_scores(conn, ticker, score_dates, momentum=50, value=50, quality=50):
    for sd in score_dates:
        conn.execute(factor_scores_table.insert().values(
            ticker=ticker, score_date=sd, sector="IT", regime="NORMAL",
            momentum_score=momentum, quality_score=quality, value_score=value,
            revisions_score=50, insider_score=50, growth_score=50,
            short_interest_score=50, institutional_score=50,
            composite_score=50, direction="NEUTRAL",
...
def _make_prices(tickers, n_days=70):
    ...
    rows = []
    for t in tickers:
        for d, c in zip(dates, [100.0 + i * 0.1 for i in range(n_days)]):
...
class TestComputeFactorReturns:
    def test_returns_dataframe_with_factor_columns(self):
        ...
        for sd in dates:
            for t in tickers:
                hist_rows.append({
                    "ticker": t, "score_date": sd,
                    "momentum_score": 90 if t == "A" else 10,
                    "quality_score": 50, "value_score": 50,
                    "revisions_score": 50, "insider_score": 50,
                    "growth_score": 50, "short_interest_score": 50,
