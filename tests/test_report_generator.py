...
from data.db import earnings_calendar, get_engine, initialise_schema, sp500_universe
...
def _seed_combined(conn, ticker, direction, score):
    conn.execute(
        combined_scores_table.insert().values(
            ticker=ticker,
...
class TestGenerateReports:
    def test_returns_empty_when_no_candidates(self, tmp_db, tmp_path):
        paths = generate_reports(
            tmp_db, "2024-06-30", {}, {}, {}, {}, output_dir=str(tmp_path / "reports")
        )
        assert paths == []
    ...
    def test_writes_file_per_candidate(self, tmp_db, tmp_path):
        ...
        paths = generate_reports(tmp_db, "2024-06-30", {}, {}, {}, {}, output_dir=out_dir)
        assert len(paths) == 2
    ...
    def test_ai_analysis_sections_included(self, tmp_db, tmp_path):
        ...
        out_dir = str(tmp_path / "reports")
        paths = generate_reports(tmp_db, "2024-06-30", ai_data, {}, {}, {}, output_dir=out_dir)
        content = Path(paths[0]).read_text()
