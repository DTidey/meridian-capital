...
from factors.short_interest import compute, COLS
...
def _make_data(tickers, si_frames=None):
    ...
    si = pd.concat(list(si_frames.values()), ignore_index=True) if si_frames else pd.DataFrame(
...
class TestLongConvention:
    def test_lower_short_pct_scores_higher(self):
        data = _make_data(["LOW_SI", "HIGH_SI"], si_frames={
    ...
    def test_declining_short_interest_scores_higher(self):
        data = _make_data(["DECLINING", "RISING"], si_frames={
    ...
    def test_lower_days_to_cover_scores_higher(self):
        data = _make_data(["EASY", "HARD"], si_frames={
...
class TestMissingData:
    ...
    def test_no_prior_30d_snapshot_si_change_is_50(self):
        ...
        data = _make_data(["AAPL", "MSFT"], si_frames={
