...
from factors.institutional import compute, COLS
...
def _inst_row(ticker, funds_holding=5, net_share_change=1e6, new_positions=0,
...
def _make_data(tickers, inst_rows=None):
    ...
    institutional = pd.DataFrame(rows) if rows else pd.DataFrame(
...
class TestStructure:
    ...
    def test_scores_0_to_100(self):
        data = _make_data(["AAPL", "MSFT", "GOOG"], inst_rows=[
    ...
    def test_latest_report_date_used(self):
        data = _make_data(["AAPL", "MSFT"], inst_rows=[
