...
def _insert_history(conn, snapshot_date, tickers, direction="LONG",
                    market_value=400_000.0, unrealized_pnl=0.0):
...
class TestCircuitBreakerForceClose:
    def test_force_close_oversized(self, mem_db, tmp_path):
        ...
        state = default_state()
        result = run_circuit_breakers(
            mem_db, _SCORE_DATE, _NAV, _base_config(max_single_position_pct=0.03),
            state, tmp_path, whatif=False
        )
