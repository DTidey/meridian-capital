...
from data.db import get_engine, initialise_schema, daily_prices, sp500_universe
    ...
    for ticker in tickers:
        ...
        for i in range(n_days):
            ...
            conn.execute(daily_prices.insert().values(
                ...
                close=price, adj_close=price, volume=500_000,
            ))
    conn.commit()
...
def _make_prices_dict(tickers, n_days=25):
    ...
    for _ in range(n_days):
        rows.append({"close": 100.0, "high": 101.0, "low": 99.0,
...
_CONFIG = {
    "portfolio": {
        "num_longs": 5, "num_shorts": 5,
        "target_long_gross": 0.90, "target_short_gross": 0.60,
        "max_position_pct": 0.30, "min_position_pct": 0.01,
        "max_sector_pct": 0.90, "max_sector_net_pct": 0.90,
        "max_beta": 0.99, "nav_usd": 1_000_000,
        "adv_lookback_days": 10, "adv_max_pct": 1.0,
        "earnings_blackout_days": 5,
        "conviction_tilt": {"top5_multiplier": 1.5, "top10_multiplier": 1.25},
        "mvo": {
            "risk_aversion": 1.0, "cov_lookback_days": 60,
...
class TestMvoOptimiser:
    def test_returns_dataframe_with_expected_columns(self, tmp_db):
        ...
        betas  = pd.Series({t: 1.0 for t in tickers})
