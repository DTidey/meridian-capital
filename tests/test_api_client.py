...
class TestChatSuccess:
    def test_returns_parsed_dict(self):
        ...
        with patch.object(
            client._client.chat.completions, "create", return_value=_mock_response(expected)
        ):
    ...
    def test_json_mode_set_by_default(self):
        client, _ = _make_client()
        with patch.object(
            client._client.chat.completions, "create", return_value=_mock_response({"ok": True})
        ) as mock_create:
...
class TestRetry:
    def test_retries_on_rate_limit(self):
        ...
        side_effects = [openai.RateLimitError("rate limit", response=MagicMock(), body={}), good]
        with (
            patch.object(client._client.chat.completions, "create", side_effect=side_effects),
            patch("analysis.api_client.time.sleep"),
        ):
            result = client.chat("sys", "user")
    ...
    def test_raises_after_all_retries_exhausted(self):
        ...
        with (
            patch.object(client._client.chat.completions, "create", side_effect=err),
            patch("analysis.api_client.time.sleep"),
    ...
    def test_sleeps_between_retries(self):
        ...
        err = openai.RateLimitError("rate limit", response=MagicMock(), body={})
        with (
            patch.object(client._client.chat.completions, "create", side_effect=[err, good]),
            patch("analysis.api_client.time.sleep") as mock_sleep,
        ):
            client.chat("sys", "user")
