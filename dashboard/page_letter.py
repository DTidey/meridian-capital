...
from dashboard.theme import ACCENT, NEUTRAL, TEXT_MUTED, section_header
...
def render(engine, cfg: dict) -> None:
    ...
    with engine.connect() as conn:
        ...
        cached = conn.execute(
            sa.select(lp_letters)
            .where(lp_letters.c.letter_date == today)
        ).fetchone()
    ...
    content     = cached[2]
    doc_id      = cached[1] or f"MCP-IM-{today[:4]}-{today[5:7]}{today[8:]}"
    ...
    st.markdown(
        ...
        f'background:#f8f6f1;'
        f'border:1px solid #d4c9b0;border-radius:8px;'
        f'padding:2.5rem 3rem;font-family:Georgia,serif;line-height:1.8;'
        f'color:#1a1a1a;">'
        + _md_to_html(full)
        + "</div>",
...
def _md_to_html(md: str) -> str:
    ...
    for line in lines:
        # Headings
        if line.startswith("# "):
            out.append(f'<h1 style="font-size:1.4rem;letter-spacing:0.05em;color:#1a1a1a;">{line[2:]}</h1>')
        elif line.startswith("## "):
            out.append(f'<h3 style="font-size:1rem;color:#5a5a5a;letter-spacing:0.08em;">{line[3:]}</h3>')
        # Horizontal rule
        elif line.strip() == "---":
