...
def render(engine, cfg: dict) -> None:
    ...
    fig.add_trace(go.Scatter(
        x=nav_df["date"], y=nav_df["port_idx"],
        name="Portfolio", line=dict(color=ACCENT, width=2),
    ))
    if "spy_idx" in nav_df.columns:
        fig.add_trace(go.Scatter(
            x=nav_df["date"], y=nav_df["spy_idx"],
            name="SPY", line=dict(color=NEUTRAL, width=1.5, dash="dot"),
        ))
    ...
    if not daily_rets.empty:
        ...
        def _colour(val):
            if val is None or (isinstance(val, float) and math.isnan(val)):
                return ""
            return f"background-color: {'rgba(16,185,129,0.2)' if val >= 0 else 'rgba(244,63,94,0.2)'}"

        styled = grid.style.map(_colour).format(
            lambda v: f"{v:+.1%}" if v is not None and not (isinstance(v, float) and math.isnan(v)) else "—"
        )
    ...
    fig2.add_trace(go.Scatter(
        x=nav_df["date"], y=-nav_df["drawdown_pct"] * 100,
        fill="tozeroy", fillcolor="rgba(244,63,94,0.15)",
        line=dict(color=SHORT_COL, width=1.5),
        name="Drawdown %",
    ))
    ...
    if not attr_df.empty:
        ...
        for col, colour, label in [
            ...
            fig3.add_trace(go.Bar(
                x=attr_90["date"], y=attr_90[col] * 100,
    ...
    if len(daily_rets) >= 252:
        roll = (daily_rets.rolling(252).mean() * 252 - 0.05) / (daily_rets.rolling(252).std() * math.sqrt(252))
        roll = roll.dropna().reset_index()
        roll.columns = ["date", "sharpe"]
        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(x=roll["date"], y=roll["sharpe"], name="Sharpe", line=dict(color=ACCENT)))
    ...
    try:
        ...
        if not sec_df.empty:
            ...
            cols = st.columns(3)
            with cols[0]: metric_card("Total Alpha (90d)", f"{total_alpha:+.2%}", LONG_COL if total_alpha >= 0 else SHORT_COL)
            with cols[1]: metric_card("Winner Sectors", str(winner_count), LONG_COL)
            with cols[2]: metric_card("Loser Sectors",  str(loser_count),  SHORT_COL)

            fig5 = go.Figure(go.Bar(
                x=sec_df["sector"], y=sec_df["alpha"] * 100,
    ...
    try:
        ...
        with c1: metric_card("30d Turnover",   f"{tv['turnover_30d_pct']:.1%}")
        with c2: metric_card("Annualised",      f"{tv['turnover_annualized']:.1%}")
        with c3: metric_card("Budget",          f"{tv['budget_pct']:.1%}", NEUTRAL)
        with c4: metric_card("Tax Estimate",    f"${tv['tax_estimate_usd']:,.0f}", SHORT_COL)
    except Exception as e:
    ...
    if trade_rows:
        ...
        disp_cols = ["ticker", "direction", "holding_days", "entry_price", "exit_price", "realized_pnl"]
    ...
    try:
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1: metric_card("Win Rate",    f"{overall.get('win_rate', 0):.1%}", LONG_COL)
        with c2: metric_card("P/L Ratio",   f"{overall.get('pl_ratio', 0):.2f}")
        with c3: metric_card("Avg Win",     f"${overall.get('avg_win', 0):,.0f}", LONG_COL)
        with c4: metric_card("Avg Loss",    f"${overall.get('avg_loss', 0):,.0f}", SHORT_COL)

        tabs = st.tabs(["By Side", "By Holding Period", "By Sector", "By VIX Regime", "By Quintile"])
