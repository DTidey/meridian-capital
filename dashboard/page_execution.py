...
def render(engine, cfg: dict) -> None:
    ...
    with engine.connect() as conn:
        # KPI data
        filled_30d = conn.execute(
            sa.select(sa.func.count()).select_from(execution_orders)
            .where(
        ...
        short_tickers = [
            r[0] for r in conn.execute(
                ...
                .where(portfolio_positions.c.direction == "SHORT")
            ).fetchall()
        ]
    ...
    with c1: metric_card("Filled Orders 30d",   str(filled_30d))
    with c2: metric_card("Avg Slippage (bps)",  f"{slip.get('mean_bps', 0):.1f}")
    with c3: metric_card("Total Slippage $",     f"${total_slip_usd:,.0f}", SHORT_COL)
    with c4: metric_card("Open Orders",         str(open_count), "#f59e0b" if open_count else NEUTRAL)
    ...
    if open_rows:
        ...
        st.dataframe(
            open_df[["rebalance_date", "ticker", "action", "ordered_shares", "filled_shares", "status", "created_at"]],
    ...
    if recent_rows:
        ...
        disp_cols = ["rebalance_date", "ticker", "action", "ordered_shares",
                     "filled_shares", "avg_fill_price", "slippage_bps", "status"]
    ...
    if worst_rows:
        ...
        st.dataframe(
            w_df[["rebalance_date", "ticker", "action", "filled_shares",
    ...
    if short_tickers:
        ...
        for t in short_tickers:
            try:
                ...
                shortable = None
            avail_data.append({"Ticker": t, "Shortable": "✓" if shortable else ("✗" if shortable is False else "?")})
    ...
    if notional_rows:
        ...
        fig = go.Figure(go.Bar(
            x=not_df["date"], y=not_df["notional"],
            marker_color=ACCENT,
        ))
        fig.update_layout(
            ...
            yaxis=dict(gridcolor="rgba(255,255,255,0.05)", title="Notional ($)"),
