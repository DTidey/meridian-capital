...
def render(engine, cfg: dict) -> None:
    ...
    cols = st.columns(4)
    with cols[0]:
        metric_card("VIX", f"{vix:.1f}" if vix else "—", SHORT_COL if vix and vix > 25 else NEUTRAL)
    with cols[1]:
        metric_card("HYG", f"{hyg:.2f}" if hyg else "—")
    with cols[2]:
        metric_card("TLT", f"{tlt:.2f}" if tlt else "—")
    with cols[3]:
        metric_card("HYG-TLT Spread", f"{spread:.2f}" if spread is not None else "—")
    ...
    with engine.connect() as conn:
        ...
        if latest_sd:
            score_rows = conn.execute(
                sa.select(fs_table.c.ticker, *[fs_table.c[c] for c in _FACTOR_SCORE_COLS]).where(
                    fs_table.c.score_date == latest_sd
                )
            ).fetchall()
        else:
            score_rows = []
    ...
    if score_rows and pos_rows:
        ...
        for fname, z in factor_z.items():
            ...
            fig.add_bar(x=[z], y=[fname], orientation="h", marker_color=colour, name=fname)
