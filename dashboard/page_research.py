...
from analysis.db import analysis_results, combined_scores
from dashboard.theme import (
    ACCENT, LONG_COL, NEUTRAL, SHORT_COL, TEXT_MUTED, TEXT_PRIMARY,
    metric_card, section_header, vix_badge,
)
from data.db import daily_prices, earnings_calendar, insert_or_replace
...
def render(engine, cfg: dict) -> None:
    ...
    with engine.connect() as conn:
        ...
        universe_size = conn.execute(
            sa.select(sa.func.count()).select_from(factor_scores_table)
            .where(factor_scores_table.c.score_date == latest_score_date)
        ).scalar() or 0 if latest_score_date else 0

        long_cands  = conn.execute(
            sa.select(sa.func.count()).select_from(factor_scores_table)
            .where(factor_scores_table.c.score_date == latest_score_date,
                   factor_scores_table.c.direction == "LONG")
        ).scalar() or 0 if latest_score_date else 0

        short_cands = conn.execute(
            sa.select(sa.func.count()).select_from(factor_scores_table)
            .where(factor_scores_table.c.score_date == latest_score_date,
    ...
    with c1: metric_card("Universe", str(universe_size))
    with c2: metric_card("Long Cands", str(long_cands),  LONG_COL)
    with c3: metric_card("Short Cands", str(short_cands), SHORT_COL)
    with c4: metric_card("Regime", str(regime), NEUTRAL)
    with c5: metric_card("Score Date", latest_score_date or "—")
...
def _advisory_banner(engine, cfg: dict) -> None:
    ...
    with engine.connect() as conn:
        held = [r[0] for r in conn.execute(
            sa.select(portfolio_positions.c.ticker)
        ).fetchall()]
...
def _approval_section(engine, cfg: dict) -> None:
    with engine.connect() as conn:
        pending = conn.execute(
            sa.select(position_approvals)
            .where(position_approvals.c.status == "PENDING")
        ).fetchall()
        ...
        if passed:
            ...
            with st.spinner("Submitting orders to Alpaca…"):
                ...
                result = subprocess.run(
                    [sys.executable, str(root / "run_execution.py")],
...
def _run_veto_checks(engine, cfg: dict, pending) -> dict[str, str]:
    ...
    with engine.connect() as conn:
        ...
        severe_events = conn.execute(
            sa.select(risk_ev.c.event_type)
            .where(risk_ev.c.event_date == today)
        ).fetchall()
        ...
        if not (net_min <= net <= net_max):
            for row in pending:
                if row[1] not in vetoes:
                    vetoes[row[1]] = f"Net exposure {net:.2%} out of bounds [{net_min:.2%}, {net_max:.2%}]"
                ...
                with st.expander(
                    ...
                    if ticker in analyses:
                        ...
                        try:
                            ...
                            st.markdown(
                                analysis_data.get("summary") or str(analysis_data)[:600]
                            )
                        except Exception:
