...
def _weighted_avg(book_df: pd.DataFrame, scores_df: pd.DataFrame) -> dict[str, float]:
    if book_df.empty:
        return dict.fromkeys(_FACTOR_COLS, 50.0)
...
def _historical_spreads(
    ...
    available = [c for c in _FACTOR_COLS if c in [col.name for col in factor_scores_table.columns]]
