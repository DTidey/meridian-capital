...
def _mvo(
    ...
    tc_cfg   = pcfg.get("transaction_costs", {})
    ...
    # Sector membership
    sector_of = dict(zip(all_cands["ticker"], all_cands["sector"].fillna("Unknown")))

    constraints, bounds = _build_constraints(
        tickers, n_long, n_short, long_gross, short_gross,
        min_pos, max_pos, max_sector, max_sector_net, max_beta,
        betas, sector_of,
    )
...
def _adjust_mu_for_costs(
    ...
    mu = mu_gross.copy()
    tilt_map = tilt_result.set_index("ticker")["weight"].abs().to_dict() if not tilt_result.empty else {}
...
def _build_constraints(
    ...
    constraints = [
        ...
        # Net beta ≤ max_beta
        {"type": "ineq", "fun": lambda w: max_beta - abs(
            sum(w[i] * betas.get(tickers[i], 1.0) for i in range(len(tickers)))
        )},
    ]
    ...
    for sector in sectors:
        ...
        if s_long:
            constraints.append({"type": "ineq",
                "fun": lambda w, sl=s_long: max_sector - sum(w[i] for i in sl)})
        if s_short:
            constraints.append({"type": "ineq",
                "fun": lambda w, ss=s_short: max_sector + sum(w[i] for i in ss)})
    ...
    bounds = (
        [(min_pos, max_pos)]   * n_long +
        [(-max_pos, -min_pos)] * n_short
    )
...
def _weights_to_df(
    ...
    for _, row in df.iterrows():
        ...
        price = float(price_df[close_col].iloc[-1]) if (not price_df.empty and close_col in price_df.columns) else 0.0
    ...
    return df[["ticker", "direction", "weight", "shares", "sector",
