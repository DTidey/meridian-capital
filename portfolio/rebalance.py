...
from portfolio.db import position_approvals, portfolio_positions
...
def generate_trades(
    ...
    cur_map = (current.set_index("ticker")["shares"].to_dict()
               if not current.empty and "shares" in current.columns else {})
    tgt_map = (target.set_index("ticker")[["shares", "direction", "combined_score"]]
    ...
    for ticker in all_tickers:
        ...
        cost  = estimate_cost(ticker, abs(delta), price, price_df, config) if abs(delta) >= _HOLD_THRESHOLD else 0.0
    ...
    return trades[["ticker", "action", "current_shares", "target_shares",
                   "delta_shares", "estimated_cost_usd", "priority", "direction", "price"]]
...
def _update_positions(
    ...
    closed_tickers = set(
        active.loc[active["target_shares"].abs() < _HOLD_THRESHOLD, "ticker"]
    )
