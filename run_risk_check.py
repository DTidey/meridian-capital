...
def _print_factor_risk(frm) -> None:
    top_factor = (
        max(frm.factor_contributions, key=frm.factor_contributions.get)
        if frm.factor_contributions
        else "—"
    )
...
def _print_pre_trade(df: pd.DataFrame) -> None:
    ...
    if rejected:
        ...
        rej_detail = (
            "  ("
            + ", ".join(f"{r['ticker']} — {r.get('reason', '?')}" for _, r in rej_rows.iterrows())
            + ")"
        )
    ...
    if reduced:
        ...
        red_detail = (
            "  ("
            + ", ".join(f"{r['ticker']} — {r.get('reason', '?')}" for _, r in red_rows.iterrows())
            + ")"
        )
...
def _print_stress(results: list, nav_usd: float) -> None:
    print(
        f"  {'Scenario':<35} {'Total P&L':>12}  {'Long':>7}  {'Short':>9}  "
        f"{'Worst Long':<10}  {'Worst Short'}"
    )
    print("  " + "-" * 100)
    for r in results:
        _total_pct = r.total_pnl_pct * 100
        long_pct = (r.long_pnl_usd / nav_usd * 100) if nav_usd else 0.0
        short_k = (
            f"${r.short_pnl_usd / 1_000:+.0f}K"
            if abs(r.short_pnl_usd) >= 1_000
            else f"${r.short_pnl_usd:+.0f}"
        )
        total_str = f"${r.total_pnl_usd:>+,.0f}"
        print(
            f"  {r.name:<35} {total_str:>12}  {long_pct:>+6.1f}%  {short_k:>9}  "
...
def run(args) -> None:
    ...
    with engine.connect() as conn:
        ...
        if args.tail_only:
            ...
            print("\n--- Pre-Trade Veto ---")
            pre_trade_results = run_pre_trade(
                conn, score_date, config, cache_dir, whatif=args.whatif
            )
            ...
            if not positions.empty:
                ...
                risk_state["risk_decomposition"] = {
                    ...
                    "factor_contributions": {
                        k: round(v, 4) for k, v in frm.factor_contributions.items()
                    },
                }
            ...
            # 3. Circuit breakers
            print("\n--- Circuit Breakers ---")
            risk_state = run_circuit_breakers(
                conn, score_date, nav_usd, config, risk_state, cache_dir, whatif=args.whatif
            )
            ...
            # 5. Factor monitor
            print("\n--- Factor Monitor ---")
            factor_alerts = run_factor_monitor(
                conn, positions, score_date, config, whatif=args.whatif
            )
            _print_factor_monitor(factor_alerts)

            # 6. Correlation monitor
            print("\n--- Correlation Monitor ---")
            corr_result = run_correlation_monitor(
                conn, positions, score_date, config, whatif=args.whatif
            )
...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stress", action="store_true", help="Run stress tests after full pipeline"
    )
    parser.add_argument(
        "--tail-only", action="store_true", dest="tail_only", help="Tail risk monitors only"
    )
    parser.add_argument(
        "--pre-trade-only", action="store_true", dest="pre_trade_only", help="Pre-trade veto only"
    )
    parser.add_argument(
        "--clear-halt",
        action="store_true",
        dest="clear_halt",
        help="Clear kill switch lock and exit",
    )
    parser.add_argument(
        "--whatif", action="store_true", help="Run all checks but do not commit changes"
    )
