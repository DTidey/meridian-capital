...
def load_scoring_data(
    ...
    logger.debug(
        ...
        len(universe), len(prices), len(funds),
    )
...
def _load_short_interest(conn: sa.engine.Connection, cutoff: date) -> pd.DataFrame:
    ...
    rows = conn.execute(
        sa.select(
            ...
            (short_interest.c.date >= start) &
            (short_interest.c.date <= score_date_str(cutoff))
        ).order_by(short_interest.c.ticker, short_interest.c.date)
    ).fetchall()
    df = pd.DataFrame(rows, columns=["ticker", "date", "short_pct_float", "short_ratio", "shares_short"])
    df["date"] = pd.to_datetime(df["date"])
    return df
...
def _load_estimates(conn: sa.engine.Connection, cutoff: date) -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows, columns=["ticker", "date", "eps_estimate_fwd", "price_target", "num_analysts"])
    df["date"] = pd.to_datetime(df["date"])
    return df
...
def _load_insider_txns(conn: sa.engine.Connection, cutoff: date) -> pd.DataFrame:
    ...
    cols = ["ticker", "insider_name", "insider_title", "transaction_code",
...
def _load_institutional(conn: sa.engine.Connection, cutoff: date) -> pd.DataFrame:
    rows = conn.execute(
        sa.select(
            ...
            institutional_summary.c.report_date <= score_date_str(cutoff)
        ).order_by(institutional_summary.c.ticker, institutional_summary.c.report_date)
    ).fetchall()
    df = pd.DataFrame(rows, columns=["ticker", "report_date", "funds_holding", "net_share_change", "new_positions"])
    if not df.empty:
